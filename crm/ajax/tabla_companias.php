...
$id_compania = isset($_POST['id_compania'])?$_POST['id_compania']:0;
$descripcion = isset($_POST['descripcion'])?$_POST['descripcion']:'';
$id_eliminar = isset($_POST['id-eliminar'])?$_POST['id-eliminar']:0;
$nuevo		 = true;

if($id_eliminar) {
	$sql   = "DELETE FROM companias WHERE id=$id_eliminar";
	$query = pg_query($conn, $sql);
}

//-- Verificamos si viene $id_ramo para crear nuevo registro o editar
if(!$id_compania && $descripcion) {
	$sql = "INSERT INTO companias(descripcion)
		    SELECT '$descripcion'
			WHERE NOT EXISTS(SELECT id FROM companias
					         WHERE UPPER(descripcion)=UPPER('$descripcion')
							)
			RETURNING id";

} elseif($id_compania && $descripcion) {
	$nuevo = false;
	$sql="UPDATE companias SET
			descripcion='$descripcion'
		  WHERE id=$id_compania
		  RETURNING id";
}
if($descripcion) {
	$query = pg_query($conn, $sql);
...
if(!$nuevo) {
?>
	<input type="hidden" name="id-compania" id="id-compania" value="<?php print $id_compania ?>" />
<?php
}
?>

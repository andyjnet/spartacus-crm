...
if($id_eliminar) {
	$sql   = "DELETE FROM sucursales WHERE id=$id_eliminar";
	$query = pg_query($conn, $sql);
}

//-- Verificamos si viene $id_etapa para crear nuevo registro o editar
if(!$id_sucursal && $descripcion) {
	$sql = "INSERT INTO sucursales(descripcion)
		    SELECT '$descripcion'
			WHERE NOT EXISTS(SELECT id FROM sucursales
					         WHERE UPPER(descripcion)=UPPER('$descripcion')
							)
			RETURNING id";

} elseif($id_sucursal && $descripcion) {
	$nuevo = false;
...
if(!$nuevo) {
?>
	<input type="hidden" name="id-sucursal" id="id-fase" value="<?php print $id_sucursal ?>" />
<?php
}
?>

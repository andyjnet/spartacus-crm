...
$id_ramo	 = isset($_POST['id_ramo'])?$_POST['id_ramo']:0;
$descripcion = isset($_POST['descripcion'])?$_POST['descripcion']:'';
$vehiculo	 = isset($_POST['vehiculo'])?($_POST['vehiculo']=='on')?1:0:0;
$id_eliminar = isset($_POST['id-eliminar'])?$_POST['id-eliminar']:0;
$nuevo		 = true;

if($id_eliminar) {
	$sql   = "DELETE FROM ramos WHERE id=$id_eliminar";
	$query = pg_query($conn, $sql);
}

//-- Verificamos si viene $id_ramo para crear nuevo registro o editar
if(!$id_ramo && $descripcion) {
	$sql = "INSERT INTO ramos(descripcion, vehiculo)
		    SELECT '$descripcion', $vehiculo
			WHERE NOT EXISTS(SELECT id FROM ramos
					         WHERE UPPER(descripcion)=UPPER('$descripcion')
							)
			RETURNING id";

} elseif($id_ramo && $descripcion) {
	$nuevo = false;
	$sql="UPDATE ramos SET
...
$sql = "SELECT id, descripcion, vehiculo
		FROM ramos
		ORDER BY descripcion";
$query= pg_query($conn, $sql);
if(!$nuevo) {
?>
	<input type="hidden" name="id-ramo" id="id-ramo" value="<?php print $id_ramo ?>" />
<?php
}
?>
...
while($fila = pg_fetch_assoc($query)) {
	...
	$url   = "id={$fila['id']}&token=$token&vehiculo={$fila['vehiculo']}&descripcion={$fila['descripcion']}";
?>

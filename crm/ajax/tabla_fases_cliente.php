...
if($id_eliminar) {
	$sql   = "DELETE FROM clientes_fases WHERE id=$id_eliminar";
	$query = pg_query($conn, $sql);
}

//-- Verificamos si viene $id_etapa para crear nuevo registro o editar
if(!$id_fase && $descripcion) {
	$sql = "INSERT INTO clientes_fases(descripcion)
		    SELECT '$descripcion'
			WHERE NOT EXISTS(SELECT id FROM clientes_fases
					         WHERE UPPER(descripcion)=UPPER('$descripcion')
							)
			RETURNING id";

} elseif($id_fase && $descripcion) {
	$nuevo = false;
	$sql = "UPDATE clientes_fases SET
...
if(!$nuevo) {
?>
	<input type="hidden" name="id-fase" id="id-fase" value="<?php print $id_fase ?>" />
<?php
}
?>
